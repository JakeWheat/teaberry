...
Fix operation of where blocks, which can be nested

ids moved to reader so it works
next step:

the check result convertor doesn't expect nested check blocks
fix it to work with these
...
future idea to move this more in language:
fun append-list(l1, l2):
   cases l1:
     | empty => l2
     | link(a,l1x) => link(a, append-list(l1x, l2))
   end
end
...
fun provides(ps):
  ...
  fun provide-em(ac, ps):
    cases ps:
      | empty => ac
      | link(provide-all, psx) => provide-em(append-list(ac,e), psx)
      | link(provide-alias(i,a), psx) =>
           # lookup i in e
           v = record-lookup-by-var(e, i)
           # add it to the ac as i, and recurse
           provide-em(record-extend-by-var(e, a, v), psx)
    end
  end
  provide-em([list:], ps)
end
...
functions needed:
lookup element in record - it's just .?
  but it's not syntax, it's a value - need a wrapper function
++ on lists
  write this in language now?
...
> tests :: T.TestTree
> tests = T.testGroup "fixwhere" [tests1, tests4]
...
> tests1 :: T.TestTree
> tests1 = T.testGroup "fixwhere1" $
>     map (\f -> T.makeTestsIO ("fixwhere" ++ f)
