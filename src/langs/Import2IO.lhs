...
Import demo 2 (simple imports) with IO in the interpreter stack
...
> evaluate :: [(String,String)] -> IO (Either String [T.CheckResult])
> evaluate s =  do
>    let x = do
...
> tests1 :: T.TestTree
> tests1 = T.makeTestsIO "imports1a" $ evaluate simpleTestScript


> tests2 :: T.TestTree
> tests2 = --trace (either show (intercalate "\n" . map show) $ parseModules importsTestScript2) $
>     let crs = evaluate $ either error id $ parseModules importsTestScript
>     in T.makeTestsIO "imports1b" crs
