...
Implement a dump desugared setting. Show how settings can be
 implemented in general

todo:

the state and stuff is threaded through the interpreter a bit weird,
review this
cleanup mess in evaluate'
add :set parsing to the main parser instead of being separate
fix it so you can have a script with :set interleaved
  (plus no :set not at top level?)
what if it's just a function? why have new syntax?
