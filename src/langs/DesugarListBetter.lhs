...
Previously: list and nothing are defined as regular data types
but they are also bootstrapped from partially already existing
ad hoc implementations

this code changes that to do it slightly more uniformly
the main thing is that the empty and link variant constructors are defined in terms
of lists, which means they are defined in terms of themselves, because the arg
to the variant constructor is a list

details:

empty = make-variant("empty", empty),
link = lam(first, rest):
         make-variant("link",
                      link(tuple({"0"; false},
                                 {"1"; "first"},
...
instead of using empty and link in the make-variant calls,
create a special make-variant-0 and make-variant-2
which take 0 and 2 args, and don't need a list value

these are only used for lists, but they could be used for any 0 or 2
rg variant constructor desugaring.


an alternative solution might be to use recursive stuff - then empty
and link could be defined in terms of themselves
...
> module DesugarListBetter (tests
>                           ,TeaberryHandle
>                           ,newTeaberryHandle
>                           ,runScript
>                           ,runFunction
