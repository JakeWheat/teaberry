...
the desugarer for data decl generates these. can consider switching to
it when it's implemented

for now:
ffi functions that will eventually be used for general data will be implemented:
make-variant too complicate
do empty and link(first,rest) as direct ffi functions


make-variant
safe-variant-name (doesn't error if called on a non variant)
variant-field-get
is-x will be implemented as functions that call safe-variant-name
  these will be generated by helper functions that will eventually be used
  by the generic data
empty and link will be implemented using make-variant
  these will be generated by helper functions that will eventually be used
  by the generic data
a.x will use generic desugaring to variant-field-get

Construct just for lists, desugared specifically in desugarer
  because other constructs are based on list construct
  this will desugar to calls to link and empty
  this will immediately be supported for users as well

this will be a demo of some of the stuff
can refactor to data decl once that is done

and then later can think about how much to put in the builtin/list
module when imports are support (which is not the same as the user
importable lists module)

tasks:

add variant values
add make-variant, safe-variant-name, variant-field-get
desugaring will add the following to the start of every file for now:
implement empty and link(first,rest) via the helper generators
parse a.first, etc.
desugar a.first to variant-field-get
implement is-X via the helper generators
