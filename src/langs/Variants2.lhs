...
Make some special cases into variants:
tuples
nothing
...
>   where
>     unpackTuple (VariantV "tuple" [("0",TextV nm), ("1",v)]) = do
>         pure (nm,v)
...
check:
  [example2:] is [list:42]
  [example2:1,2,3] is [list:42,1,2,3]
  [example2:1,2,3,4,5,6,7,8] is [list:8,1,2,3,4,5,6,7,8]
end
...
> tests :: T.TestTree
> tests = T.testGroup "variants2" [tests1,tests2,tests3, tests4]

> tests1 :: T.TestTree
> tests1 = T.makeTestsIO "variants2a" $ pure (evaluateWithChecks simpleTestScript)

> tests2 :: T.TestTree
> tests2 = T.makeTestsIO "variants2b" $ pure (evaluateWithChecks listTests)

> tests3 :: T.TestTree
> tests3 = T.makeTestsIO "variants2c" $ pure (evaluateWithChecks constructTests)

> tests4 :: T.TestTree
> tests4 = T.makeTestsIO "variants2d" $ pure (evaluateWithChecks tupleTests)
