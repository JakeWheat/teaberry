...
Try to target the final list implementation

It uses an actual regular data decl to make the list type
but the empty and link variant makers are hardcoded already
The list construct is also hardcoded

this seems to be enough to bootstrap it
...
> tests :: T.TestTree
> tests = T.testGroup "lists2" [tests1,tests2]

> tests1 :: T.TestTree
> tests1 = T.makeTestsIO "lists2a" $ pure (evaluateWithChecks simpleTestScript)

> tests2 :: T.TestTree
> tests2 = T.makeTestsIO "lists2b" $ pure (evaluateWithChecks listTests)
