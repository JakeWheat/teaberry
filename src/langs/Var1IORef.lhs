...
simplest statements thing with vars added, changed to use iorefs in
the reader env for var representation instead of the dodgily
implemented store
...
this is a problem with the current implementation - leaving the repl
idling gradually uses up all the memory on the machine. Not quite sure
it's down to non garbage collected store, but can try to get to the
bottom of it starting with this code:

create vars in a loop, they become unreachable in the next loop
iteration

try to do this many many times and see that the memory use doesn't
grow unbounded

then update a var in a loop, check again that the memory use doesn't
grow unbounded

figure out how to make these into automated tests

then do a test in the updated front or something which checks the repl
doesn't continually grow in its use of memory in case it's because of
something else - other options dangling references and too much
laziness, maybe it's not even one of these causing it

make a new exe for all of this for now
...
> interp (SetBox b _) = throwInterp $ "attemped to setbox non identifier: " ++ pretty b
...
> evaluateWithChecks :: String -> IO (Either String [T.CheckResult])
...
> tests :: T.TestTree
> tests =
>     let crs = evaluateWithChecks simpleTestScript
>     in T.makeTestsIO "var1IORef" crs
