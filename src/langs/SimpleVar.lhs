...
DONE work out the desugarer stack changes
implement the desugarer parts
...
DONE var decl
DONE var assign
deboxing vars is implicit for the user

desugaring:
track vars in desugarer effects
vardecl -> letdecl box + add to list
setvar -> setbox
iden -> if var, then unbox, otherwise just iden

interpreter:
DONE new syntax
box values
add store to interpreter stack
  + api
+ interp for the introduced syntax - box, setbox, unbox
